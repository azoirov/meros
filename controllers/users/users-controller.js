...
module.exports = class UsersController {
        ...
        try {
            ...
            if (Number(code) !== Number(attempt.code)) {
                ...
                if (
                    ...
                    Number(settings.code_attempts) - 2
                ) {
                    await attempts.destroy({
                        where: {
                            attempt_id: code_validation_id,
    ...
    static async EditUserFullName(req, res) {
        try {
            const { firstName, lastName } = await editFullNameValidation.validateAsync(req.body);
    ...
    static async EditPhoneEmail(req, res) {
        try {
            const { phone, email } = await editEmailPhoneValidation.validateAsync(req.body)
        ...
        try {
            if (req.files) {
                ...
                if (type !== "image" && type !== "vector") {
                    throw new Error(
                        "avatar image type must be an image or svg vector"
                    );
                }
