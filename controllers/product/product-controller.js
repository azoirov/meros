...
module.exports = class ProductsController {
      ...
      if (!files) {
        ...
        for (let i in files) {
          if (
            ...
            throw new Error("Invalid file, file must be type of image");
          }
  ...
  static async CategoryGetController(req, res) {
    try {
      ...
      sub_categories = sub_categories.map((el) => {
        el.sub_sub_categories = sub_sub_categories.filter(
          (c) => c.sub_category_id === el.sub_category_id
        );
        return el;
      });
  ...
  static async ProductsGetController(req, res) {
    try {
      ...
      let products = await req.db.products.findAll({
        where: {
          sub_sub_category_id: sub_sub_category.dataValues.sub_sub_category_id,
        },
        raw: true,
      ...
      let home_banners = await fs.readFile(
        path.join(__dirname, "..", "..", "banners.json"),
        { encoding: "utf-8" }
      );
      ...
      res.render("category", {
        title:
          "Meros | " + sub_sub_category.dataValues.sub_sub_category_name_ru,
        user: req.user,
