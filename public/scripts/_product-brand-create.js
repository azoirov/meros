...
export default function () {
   try {
      const productBrandCreateForm = selectOne('#product-brand-create-form'),
         productBrandNameInput = productBrandCreateForm.querySelector('#product-brand-name'),
      ...
      productBrandEditBtns.forEach(el => {
         el.addEventListener('click', async e => {
            let response = await fetch('/admin/product-brands/' + e.currentTarget.id)
            response = await response.json()
            modalContent.innerHTML = `
      ...
      function productBrandEditSubmit(id) {
         const productBrandEditForm = selectOne('#product-brand-edit-form'),
            productBrandNameInput = productBrandEditForm.querySelector('#product-brand-name')
