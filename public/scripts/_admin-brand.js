...
export default function adminBrand() {
   try {
      ...
      editBrandBtns.forEach(el => {
         el.addEventListener('click', async ev => {
            ...
            response.result.categories.forEach(category => {
               selectOptions += `
                  <option ${ response.result.brand.category_id === category.category_id ? 'selected' : '' } value="${category.category_id}">${category.uz_name}</option>
               `
            })
      ...
      function editBrandImage() {
         const brandEditFileInputs = selectAll('#brand-edit-form input[type="file"]')
         brandEditFileInputs.forEach(el => {
            el.addEventListener('change', ev => {
               const brandImg = ev.target.parentElement.parentElement.previousElementSibling.children[0]
      ...
      function brandImgUpdatedRemove() {
         const brandImgUpdatedVersionRemoveElements = selectAll('#brand-img-update-version-remove')
         brandImgUpdatedVersionRemoveElements.forEach(el => {
            el.addEventListener('click', e => {
               const brandImgFileInput = e.currentTarget.parentElement.parentElement.parentElement.nextElementSibling.children[1].children[1]
