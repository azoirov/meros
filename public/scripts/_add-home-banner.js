...
export default function () {
  try {
    const homeBannerForm = selectOne("#home-banner-form"),
      ...
      bannerHorizontal1 = homeBannerForm.querySelector(
        "#home-banner-horizontal-1"
      ),
      bannerHorizontal2 = homeBannerForm.querySelector(
        "#home-banner-horizontal-2"
      ),
      homeBannerLeftURL = homeBannerForm.querySelector("#home-banner-left-url"),
      homeBannerRightURL = homeBannerForm.querySelector(
        "#home-banner-right-url"
      ),
      homeBannerSmall1URL = homeBannerForm.querySelector(
        "#home-banner-small-1-url"
      ),
      homeBannerSmall2URL = homeBannerForm.querySelector(
        "#home-banner-small-2-url"
      ),
      homeBannerHorizontal1URL = homeBannerForm.querySelector(
        "#home-banner-horizontal-1-url"
      ),
      homeBannerHorizontal2URL = homeBannerForm.querySelector(
        "#home-banner-horizontal-2-url"
      ),
    ...
    homeBannerForm.addEventListener("submit", async (e) => {
      ...
      if (sliderItemsWrapper.children.length > 0) {
        ...
        sliderItems.forEach((item, index) => {
          if (
            item.firstElementChild.firstElementChild.children[2].files[0] &&
            item.children[1].firstElementChild.children[1].value
          ) {
            formData.append(
              `carousel-${index}`,
              item.firstElementChild.firstElementChild.children[2].files[0]
